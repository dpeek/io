...
description: "Phase-1 local dev runtime, project bootstrap, auth, routes, and browser opening owned by @dpeek/graphle-local."
...
- you are adding or changing phase-1 `/api/*` routes

## Runtime Flow

`graphle dev` prepares the current working directory, opens the SQLite file,
starts a loopback Bun HTTP server, and opens the browser through `/api/init`.
...
- `GET /api/health`: service status plus SQLite health
...
All non-API routes return the placeholder public-site HTML in this phase.
