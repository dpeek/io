...
test("buildAgentTuiRootComponentModel renders a single human-readable block stream", () => {
  ...
  store.observe({
    ...
    sequence: 5,
    session: worker,
    text: "|  M lib/app/src/agent/runner/codex.ts",
  ...
  expect(content).toContain("   M lib/app/src/agent/runner/codex.ts");
  ...
  expect(snapshotColumn?.blocks.find((entry) => entry.kind === "command")).toMatchObject({
    kind: "command",
    outputLines: ["## main", " M lib/app/src/agent/runner/codex.ts"],
  });
});
...
test("buildAgentTuiRootComponentModel keeps live transcript slices readable in replayable snapshots", () => {
  ...
  store.observe({
    ...
    params: {
      delta:
        "lib/app/src/agent/runner/codex.ts | 4 ++--\nlib/app/src/agent/tui/ui.test.ts | 8 ++++++--\n",
      itemId: "cmd-1",
    },
    sequence: 3,
    session: worker,
  ...
  expect(liveContent).toContain("  lib/app/src/agent/runner/codex.ts | 4 ++--");
