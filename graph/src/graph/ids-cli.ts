...
import { createIdMap, extractSchemaKeys, findDuplicateIds, type IdMap } from "./identity";
...
function usage(): never {
  console.error(
    [
      "Usage:",
      "  bun run src/ids-cli.ts check <schema-file.ts>",
      "  bun run src/ids-cli.ts sync <schema-file.ts> [--prune-orphans]",
      "  bun run src/ids-cli.ts rename <schema-file.ts> <old-key> <new-key>",
      "",
...
async function main(): Promise<void> {
  const [, , command, schemaPathInput, ...rest] = process.argv;
...
main().catch((error) => {
  ...
  console.error(`[${origin}] ${error instanceof Error ? error.message : String(error)}`);
  process.exit(1);
});
