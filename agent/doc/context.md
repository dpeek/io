...
- `io.core.git-safety`
- `io.core.validation`
- `io.agent.execute.default`
- `io.agent.backlog.default`
- `io.linear.issue-routing`
- `io.context.resolution`

### Project docs

Project docs stay in the repo and are registered in `io.json`.
...
```json
{
  ...
  "context": {
    ...
    "profiles": {
      ...
      "backlog": {
        "include": ["builtin:io.agent.backlog.default", "builtin:io.core.git-safety", "./io.md"]
      }
    }
  },
  "issues": {
...
Routing rules should inspect stable issue metadata such as:

- labels
- state
- parent/sub-issue status
- project
- title prefixes only if needed

Prefer labels over title parsing.
...
For that, support a small issue-body block that `io` can parse.
...
- whether built-in docs use bare ids like `io.agent.execute.default` or explicit ids like `builtin:io.agent.execute.default`
- whether `io.md` is always injected automatically or can be excluded by a profile
- whether issue-body hint syntax should be HTML comment based or fenced-code based
- whether project docs must be registered in `io.json` before path references are allowed
- whether routing should consider only labels at first, or also issue hierarchy
...
- always include `io.md` unless a profile explicitly disables it
- allow both registered doc ids and repo-relative paths
- start routing with labels only
- support issue-body hints, but do not require them for normal use
...
- `agent/src/tracker/linear.ts`
  - grow issue fetches to include whatever metadata routing needs first
